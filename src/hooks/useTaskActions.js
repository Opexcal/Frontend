...
export const useTaskActions = (onSuccess) => {
  ...
  const acceptTask = async (taskId) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.message || error.message || 'Failed to accept task',
        variant: 'destructive',
      })
    } finally {
      setLoading(false)
  ...
  const rejectTask = async (taskId, rejectionReason) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.message || error.message || 'Failed to reject task',
        variant: 'destructive',
      })
    } finally {
      setLoading(false)
